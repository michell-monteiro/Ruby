class Cart
    ...
    def initialize(attributes = {})
        ...
        @items = [] #Iniciamos a sacola vazia. Cada item aqui é um objeto
        @shipping_cost  = nil
    ...
    def show
        return puts 'Carrinho vazio' if items.empty? #Se itens for vazio, imprimir mensagem
    ...
    def add(item)
        @items.push(item) #Funciona também @item << item
        refresh_shipping_cost #Atualiza e calcula o preço do frete
    end
    ...
    #Validação 
    def checkout
        puts validate_checkout if validate_checkout.any?
    ...
    def validate_checkout
        [].tap do |errors|
            errors.push('Cliente não enviado ou inválido') unless customer && customer.class == Customer #Verificando se o objeto é da classe cliente e se ele foi instanciado
    ...
    def partial_amount
        items.sum(&:price) #Soma o valor de cada item, através do price
    end
