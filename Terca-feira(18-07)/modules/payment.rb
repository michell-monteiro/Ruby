module Payment
    def pay(params = {})
        ...
        return puts ' O valor mínimo não foi antingido' unless amount > 1
    ...
    def credit_card(options = {})
        ...
        if card_number && card_cvv && card_holder && card_expiry
            puts "Pago com sucesso com o cartão de crédito #{card_holder}."
        else
            ...
            puts 'Data de validadew co cartão não enviada' unless card_expiry
        end
    end
    ...
    def bank_slip(options = {})
        ...
        if name && document && email
            puts "Boleto enviado para #{email}"
        else
            puts 'Nome não enviado' unless name
            puts 'CPF não enviado' unless document
